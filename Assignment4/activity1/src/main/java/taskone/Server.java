        ...
        try {
            ...
            while (!quit) {
                try {
                    ...
                    // If an error occurs, send a proper error response to the client
                    JSONObject errorResponse = Performer.error("Invalid request format or unexpected data.");
                    byte[] errorOutput = JsonUtils.toByteArray(errorResponse);
                    NetworkUtils.send(out, errorOutput);
                    e.printStackTrace();
                }
